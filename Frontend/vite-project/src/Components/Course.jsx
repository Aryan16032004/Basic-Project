...
import databaseService from "../BackendApi/database";
import { Container } from "../Components";
import {Topic} from "./index.js"; // Import the Topic component
...
export default function Course() {
    ...
    useEffect(() => {
        const fetchTopics = async () => {
            if (id) {
                try {
                    const response = await databaseService.getUserTopics(id); // Fetch topics by course ID
                    const topicData = response.data.data ?? []; // Defensive check for data
                    console.log(topicData);
                    
                    if (Array.isArray(topicData)) {
                        setTopics(topicData); // Set the topics in the state
                    } else {
                        console.error("Invalid data format received", topicData);
                    }
                } catch (error) {
